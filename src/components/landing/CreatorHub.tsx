...
const CreatorHub = () => {
  ...
  // Formatting helpers for display (keeps UI readable and deterministic)
  const fmtNumber = (n: number) => n.toLocaleString();
  const fmtCurrency = (n: number) => `$${n.toLocaleString()}`;
  const fmtPercent = (v: number) => `${(v * 100).toFixed(1)}%`;

  return (
    <section className="py-16 md:py-24 bg-[#0F172A]/30">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          ...
          {/* Featured Creator - Hero Card (spans 2 cols, 2 rows) */}
          <motion.div
          ...
          <Card className="overflow-hidden border-white/10 bg-gradient-to-br from-primary/10 to-accent/5 h-full backdrop-blur-sm">
            <CardContent className="p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="font-semibold text-white">{t("creatorHub.liveActivity.title")}</h4>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-white/10 grid grid-cols-3 gap-4 text-center">
                <div>
                  ...
                  <p className="text-xs text-slate-400">{t("creatorHub.stats.activeCreators")}</p>
                </div>
                <div>
                  <p className="text-xl font-bold text-primary">$2.4M</p>
                  <p className="text-xs text-slate-400">{t("creatorHub.stats.paidOut")}</p>
                </div>
                <div>
                  <p className="text-xl font-bold text-white">850+</p>
                  <p className="text-xs text-slate-400">{t("creatorHub.stats.brandPartners")}</p>
                </div>
              </div>
            </CardContent>
          </Card>
